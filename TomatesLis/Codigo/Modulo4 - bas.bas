...
'
    ...
    Range("D3").Select
    ActiveWorkbook.Worksheets("NOMINA").ListObjects("NOMINA").Sort.SortFields. _
        Clear
    ActiveWorkbook.Worksheets("NOMINA").ListObjects("NOMINA").Sort.SortFields. _
        Add2 Key:=Range("NOMINA_[FINCA]"), SortOn:=xlSortOnValues, Order:= _
 ...
 Do While True
   If ActiveCell.Value = actFinca Then
     Do While ActiveCell.Value = actFinca
       If ActiveCell.Offset(0, 4).Value = "" Or CVDate(ActiveCell.Offset(0, 4).Value) > CVDate("01/" & el_mes & "/" & el_anho) Then 'ACTIVO
...
Sub Inserta_Linea(CeldaTotalesFinca As String)
 Dim lin, Almacen, LaTorre, GobernI, GobernII, color_fondo As Double
 Dim lin_Char, celda_back, rng_Sel  As String
 Almacen = 13819130
 LaTorre = 13826780
 GobernI = 16440530
GobernII = 13172735
 Select Case CeldaTotalesFinca
 ...
 Else
  ...
  'Range("A" & ActiveCell.Row & ":AF" & ActiveCell.Row).Interior.Color = color_fondo
